...
namespace nc
{
	...
	void CameraComponent::SetProgram(res_t<Program> program)
	{
		...
		program->SetUniform("projection", &view);
	}

	void CameraComponent::ProcessGui()
	{
		// use ImGui::DragFloat to set fov, aspect, near and far values (use speed of 0.1f)
		ImGui::DragFloat("Field of View", &fov, 0.1f, 35.0f, 120.0f);
		ImGui::DragFloat("Near View", &near, 0.1f, 0.1f, far);
		ImGui::DragFloat("Far View", &far, 0.1f, near, 1000.0f);
	}
